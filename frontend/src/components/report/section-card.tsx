...
function Block({ block }: { block: ReportBlock }) {
  if (block.kind === 'text') {
    return (
      <pre className='text-foreground bg-muted/30 whitespace-pre-wrap rounded-md p-3 font-sans text-sm leading-relaxed'>
        {block.body}
      </pre>
    );
  }
          ...
          <tbody>
            {block.rows.map((row, ri) => (
              <tr key={ri} className='even:bg-muted/20'>
                {row.map((cell, ci) => (
                  <td key={ci} className='border-b px-3 py-2 tabular-nums'>
                    {cell}
                  </td>
                ))}
              </tr>
