...
export default async function SessionAttendancePage({
  ...
  // 출결판 표시: 'selected', 'same_day_cancel'(당일취소), 'cancel_notice'(취소통보, 미확정).
  // 제외: 'cancel_confirmed'(취소확정)·'rejected'.
  const appStatusByApplicant = new Map(
    (appRes.data ?? []).map((a) => [a.applicant_id, a.status])
  );
  const ATTENDANCE_VISIBLE = new Set(['selected', 'same_day_cancel', 'cancel_notice']);
  const studentRows = (studentRes.data ?? []).filter((s) => {
    ...
    return ATTENDANCE_VISIBLE.has(status);
  });
