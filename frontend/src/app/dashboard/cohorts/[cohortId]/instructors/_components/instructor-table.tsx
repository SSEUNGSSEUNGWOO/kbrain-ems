...
export function InstructorTable({ cohortId, instructors }: Props) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  const handleDelete = (id: string, name: string) => {
    if (!confirm(`강사 "${name}"을(를) 삭제하시겠습니까? (다른 cohort에서도 같이 삭제됨)`)) return;
    startTransition(async () => {
      const result = await deleteInstructor(cohortId, id);
      if (result?.error) {
        alert(result.error);
        return;
      }
